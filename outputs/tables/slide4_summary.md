...
**Key insights:**
- Observed **engagement fatigue**: users with **≥5 events in the last 14 days** had an outcome rate of ~49.7% vs ~55.6% for **3 events** (-5.9 pp).
- **Timing sensitivity**: when the most recent gap increases from **1d** to **14d**, average predicted probability shifts from ~59.1% to ~58.1% (-1.0 pp).
- **Counterfactual test**: adding a recent **content_review** event (gap=2d) changes average predicted probability from ~59.0% to ~59.3% (+0.3 pp).
